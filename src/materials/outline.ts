...
void main() {
  ...
  // Set the outline color to red (hue = 0.0, full saturation = 1.0) with fading alpha
  vec3 outlineColorHSL = vec3(0.0, 1.0, 1.0);  // Red color, full saturation, full brightness
  ...
  for (float i = 0.0; i < TAU; i += TAU / steps) {
    ...
    fragColor = mix(fragColor, vec4(hsv2rgb(outlineColorHSL), fadeAlpha), alpha); // apply outline with fading alpha
  }
...
export function getOutlineMaterial(engine: Engine): Material {
    if (!outlineMaterial) {
